...
document.querySelectorAll('a[href]').forEach(a => {
  ...
  if (a.closest('.mobile-nav-overlay')) return;
...
// ── SKILLS MARQUEE NATIVE AUTO-SCROLL ──
...
if (skillsWrap && skillsTrack) {
  // 1. Get original content and compute loop width
  const originalHTML = skillsTrack.innerHTML;
  let gap = parseFloat(getComputedStyle(skillsTrack).gap) || 0;
  let loopWidth = skillsTrack.scrollWidth + gap;
  
  // 2. Duplicate content enough times to ensure seamless infinite scroll in both directions
  while (skillsTrack.scrollWidth < loopWidth + window.innerWidth * 2) {
    skillsTrack.innerHTML += originalHTML;
  }
  ...
  // 3. Setup drag to scroll (mouse)
  skillsWrap.addEventListener('mousedown', (e) => {
    isDragging = true;
    skillsWrap.style.cursor = 'grabbing';
    startX = e.pageX - skillsWrap.offsetLeft;
    scrollLeftStart = skillsWrap.scrollLeft;
  });
  
  window.addEventListener('mouseup', () => {
    isDragging = false;
    skillsWrap.style.cursor = '';
  });
  ...
  window.addEventListener('mousemove', (e) => {
    ...
    skillsWrap.scrollLeft = scrollLeftStart - walk;
  });

  // Support vertical mouse wheel scrolling to horizontal scroll
  skillsWrap.addEventListener('wheel', (e) => {
    if (e.deltaY !== 0) {
      skillsWrap.scrollLeft += e.deltaY;
    }
  }, { passive: true });

  // Pause on hover or touch
  skillsWrap.addEventListener('mouseenter', () => { isPaused = true; skillsWrap.style.cursor = 'grab'; });
  skillsWrap.addEventListener('mouseleave', () => { isPaused = false; isDragging = false; skillsWrap.style.cursor = ''; });
  skillsWrap.addEventListener('touchstart', () => isPaused = true, { passive: true });
  skillsWrap.addEventListener('touchend', () => isPaused = false, { passive: true });

  // 4. Auto-scroll and infinite loop logic
  function autoScroll() {
    // Move left to right (decreasing scrollLeft)
    if (!isPaused && !isDragging) {
      skillsWrap.scrollLeft -= speed;
    }
    
    // Wrap around logic for both directions
    if (skillsWrap.scrollLeft <= 0) {
      skillsWrap.scrollLeft += loopWidth;
    } else if (skillsWrap.scrollLeft >= loopWidth) {
      skillsWrap.scrollLeft -= loopWidth;
    }
    
    requestAnimationFrame(autoScroll);
  }
  // Start with a valid scroll position
  setTimeout(() => {
    skillsWrap.scrollLeft = loopWidth;
    autoScroll();
  }, 100);
